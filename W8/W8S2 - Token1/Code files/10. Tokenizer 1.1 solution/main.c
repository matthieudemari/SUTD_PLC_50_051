...
int main() {
    ...
    */
    ...
    while (lexeme != NULL) {
        ...
        // But to be honest there are many many more!
         if (is_keyword_strcmp(lexeme) != TOKEN_UNKNOWN) {
        ...
        } else if (is_operator_strcmp(lexeme) != TOKEN_UNKNOWN) {
            token_type = is_operator_strcmp(lexeme);
        } else if (is_punctuation_strcmp(lexeme) != TOKEN_UNKNOWN) {
            token_type = is_punctuation_strcmp(lexeme);
			...
			// Error message
			//printf("Error: Lexeme '%s' at line '%d' is UNKNOWN.\n", lexeme, line_number);
			printf("Error: Lexeme '%s' is UNKNOWN.\n", lexeme);
        }
